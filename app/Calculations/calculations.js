...
//
// Triggers:
//   bindBOQQuantityTrigger() — fires on Quantity input (col 5) in #boqSubform
//   bindBOQRatesTrigger()    — fires on Rates    input (col 8) in #boqSubform
//
// Both triggers call the shared runSCSCalc() engine.
//
// Author:  Converted from Dinesh M. Zoho Deluge
//          SCS-Calc-Trigger-BOQ-Qty  |  SCS-Calc-Trigger-BOQ-Rate
// ============================================================
...
const BOQ_COL = {
    LINE        : 1,
    QUANTITY    : 5,   // ← trigger: bindBOQQuantityTrigger
    GIRTH       : 6,
    THICKNESS   : 7,
    RATES       : 8,   // ← trigger: bindBOQRatesTrigger
    DISCOUNT    : 9,
...
// ────────────────────────────────────────────────────────────
// RATES TRIGGER  (SCS-Calc-Trigger-BOQ-Rate)
// Fires runSCSCalc() ONLY when the user edits the Rates column
// (col 8 = BOQ_COL.RATES) in #boqSubform.
//
// Mirrors Deluge: SCS-Calc-Trigger-BOQ-Rate
//   - Null-guards row.Rates (treats blank as 0)
//   - Same full SCS_Calc pipeline as the Quantity trigger
//
// Uses event delegation → works for static and dynamic rows.
// ────────────────────────────────────────────────────────────
function bindBOQRatesTrigger() {
    const boqTbody = document.querySelector('#boqSubform tbody');
    if (!boqTbody) {
        console.warn('[Calculations] #boqSubform tbody not found — Rates trigger not bound.');
        return;
    }
    ...
    boqTbody.addEventListener('input', function (e) {
        ...
        // Fire ONLY on the Rates column (col 8)
        if (colIndex === BOQ_COL.RATES) {

            // Mirror Deluge null-guard: treat blank Rates as 0
            const ratesInput = e.target;
            if (ratesInput.value === '' || ratesInput.value === null) {
                ratesInput.value = 0;
            }

            console.log('[Calculations] BOQ Rates changed — running SCS Calc...');
            runSCSCalc();
        }
    });

    console.log('[Calculations] BOQ Rates trigger bound successfully.');
}
